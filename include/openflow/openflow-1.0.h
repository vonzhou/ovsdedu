...
enum ofp10_action_type {
    ...
    OFPAT10_FP_UPDATE,           /*update fingerprint table of this SW*/
...
/*vonzhou
* the action of updating the fingerprint table
*/
struct ofp10_action_fp_update {
    ovs_be16 type;                  /* OFPAT10_FP_UPDATE. */
    ovs_be16 len;                   /* Length is 520. */
    uint8_t pad[4];
    ovs_be32 vector[128];           /*FIXME bloomfilter vector is 4096 bits*/
};
OFP_ASSERT(sizeof(struct ofp10_action_fp_update) == 520);
...
/*-vonzhou
** this is what SDN controller send to update the fp table in the switch
*/
struct ofp_fp_update{
      ovs_be32 vector[128];
};
OFP_ASSERT(sizeof(struct ofp_fp_update) == 512);
