...
void ovs_vport_receive(struct vport *vport, struct sk_buff *skb)
{
	...
	//printk(KERN_INFO "ovs_vport_receive, vport_ops:%d\n",vport->ops->type);//vonzhou:20140327
...
/*	struct sw_flow_key key;
	key = OVS_CB(skb)->flow->key;
	printk(KERN_INFO "ovs_vport_send:%dB ,vport_ops:%d,sw_flow_key:ip_proto=%d\n",
					sent,vport->ops->type,key.ip.proto);//vonzhou:20140327
*/
	if (likely(sent)) {
		struct vport_percpu_stats *stats;
