...
void ovs_dp_process_received_packet(struct vport *p, struct sk_buff *skb)
{
	...
	if (!OVS_CB(skb)->flow) {
		...
		/* Look up flow. */
		// vonzhou20140415,I just want every packet go to userspace 
...
static int ovs_packet_cmd_execute(struct sk_buff *skb, struct genl_info *info)
{
	...
	int err;
	int key_len;
	char buffer[1500]; // just for debug,its too small for packet data actually.
	...
	//printk(KERN_INFO "alloc skb len:%d",NET_IP_ALIGN + len);
	//vonzhou: I want to get the playload,60 ??TODO
	/* if(len > 60){
		skb_copy_bits(packet,60,(void *)buffer,len - 60);
		printk(KERN_INFO "playload:%c",ntohs(buffer[1]));
	}
	*/
	err = -ENOMEM;
	if (!packet)
	...
	//printk(KERN_INFO "skb linear len:%d",skb_headlen(packet)); //
	// here we get the packet back from userspace(maybe changed)
//	memcpy((void *)tmp, nla_data(a[OVS_PACKET_ATTR_PACKET]), len);	
//	if(len>60) printk(KERN_INFO "the data kernel get from userspace:%c",tmp[61]);
	skb_reset_mac_header(packet);
	eth = eth_hdr(packet);
	//printk(KERN_INFO "if have paged data:%d",skb_is_nonlinear(packet));
	...
	//printk(KERN_INFO "eth protol:%x",packet->protocol);
	...
	if (err)
		goto err_flow_free;
	// I want to just care about TCP and calculate The TCP checksum;
	//printk(KERN_INFO "******************");
	
	//printk(KERN_INFO "packet len:%d, proto:%d ", len, flow->key.ip.proto);
	//printk(KERN_INFO "packet len:%d, proto:%d, dst_port:%d", len, flow->key.ip.proto, flow->key.ipv4.tp.dst);
	
	if(len > 42 && flow->key.ip.proto == IPPROTO_UDP && ntohs(flow->key.ipv4.tp.dst) == 2500){
		skb_copy_bits(packet, 42,(void *)buffer,len - 42);
		//printk(KERN_INFO "csum:%x",ntohs(packet->csum));
		//printk(KERN_INFO "playload:%c",buffer[0]);
		struct ethhdr *eh = (struct ethhdr *)eth_hdr(packet);
		struct iphdr *iph = (struct iphdr *)skb_network_header(packet);
		struct udphdr *udph = udp_hdr(packet);
		int source_port = ntohs((unsigned short int)udph->source);
		int dest_port = ntohs((unsigned short int)udph->dest);
		int datalen = packet->len - iph->ihl * 4 - ETH_HLEN;
		//printk(KERN_INFO "UDP header dest port:%d , udp->check before:%x",dest_port, ntohs(udph->check));
		//printk(KERN_INFO "IP iph:%d",iph->ihl);
		// next stop I will recaculate the tcp checksum (um);FIXME
		udph->check = 0;
	
		//tcph->check = tcp_v4_check(datalen, iph->saddr, iph->daddr, 
		//			csum_partial(tcph, datalen, 0)); 
	
	//	udph->check = csum_tcpudp_magic(iph->saddr, iph->daddr, datalen, iph->protocol, csum_partial((char *)udph, datalen, 0));
	
		packet->ip_summed = CHECKSUM_UNNECESSARY;
		ip_send_check(iph);
	
		//printk(KERN_INFO "udp check after:%x",ntohs(udph->check));
	}
