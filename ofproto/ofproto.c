               ...
               struct ofproto **ofprotop)
{
    ...
    *ofprotop = NULL;
	...
	ofproto->bloom=NULL; //we add our field in ofproto,need to initilize
...
ofproto_run(struct ofproto *p)
{
    ...
    struct ofport *ofport;
    int error;
   /** initialization the bloomfilter   */
     int i;
	if( p->bloom==NULL) {
         p->bloom=bloom_create(4096,0x03,RSHash,JSHash,PJWHash);
         i=0;
         while(i<512)
        {
          p->bloom->a[i]=0x00;
...
/*vonzhou
 * deal with the update fingerprint table msg from controller 
 */
static enum ofperr
...
{ 
    ...
    unsigned  char *debug;
    struct ofproto *ofproto=ofconn_get_ofproto(ofconn);
	struct ofp_fp_update *fu=ofpmsg_body(oh);
	VLOG_INFO("the recved vector in ovs : %x", fu->vector[0]);
    bloom_update(ofproto->bloom,(unsigned char *)(fu->vector));
...
handle_openflow__(struct ofconn *ofconn, const struct ofpbuf *msg)
{
    ...
    case OFPTYPE_FP_UPDATE:                  /*updata the bloomfliter*/
