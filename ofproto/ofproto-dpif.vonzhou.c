                        ...
                        struct ofpbuf *packet, const struct flow *flow)
{
    ...
    if (rule->up.cr.priority == FAIL_OPEN_PRIORITY) {
         ...
         */
	VLOG_INFO("handle_flow_miss_common invoked send_packet_in_miss");
...
/* filter the udp gram we want to monitor and dedu
*/
static int filter_flow(struct flow_miss *miss)
{
	struct flow flow = miss -> flow;
	ovs_be32 nw_dst = flow.nw_dst;
	ovs_be16 tp_dst = flow.tp_dst;
	unsigned char *ipp = (unsigned char *)&nw_dst;
	uint8_t proto = flow.nw_proto;
	if(proto == 17 && ipp[0]==192 && ipp[1]==168 && ipp[2]==4 && ipp[3]==135 && ntohs(tp_dst) == 2500) 
		return 1;
	return 0;
}
                 ...
                 struct flow_miss_op *ops, size_t *n_ops)
{
	...
	//VLOG_INFO("flow miss tp_dst is %d", flow.tp_dst);
  	/*delete the deduplication date from the packets */   
    flow = miss->flow;
    //proto = flow.nw_proto; // network protocol

	//get the ip from the struct flow
	unsigned char *iphead = (unsigned char *)(&flow.nw_dst);
	//VLOG_INFO("dst port : %d", ntohs(flow.tp_dst));
	/*TODO filter the flow we want to monitor*/
	if(filter_flow(miss)){
		VLOG_INFO("== packet to server ==");
	   	LIST_FOR_EACH (packet, list_node, &miss->packets) {
			/*deal with every packet of this kind of flow*/
			dstip = *((unsigned int *)(packet->l3)+4);
            iphead=(unsigned char *)(&dstip);
			k = packet->size;
			//VLOG_INFO("packet size : %d ", k);
			
	      	len = *((unsigned short int *)(packet->l3)+1);		// ip gram length
            //len=htons(len);
			//VLOG_INFO("after ntohs , IP gram len : %d", ntohs(len));
            playload = packet->l7;
            oneswitch = &(ofproto->up);  // get the ofproto instance
			
			/*TODO check bloom filter according to the fp and flag, then dede */
			
			for(j = 0; j< 20; j++)
				VLOG_INFO("fp: %x", *(char *)(playload + j));
			VLOG_INFO("bloom check : %d", bloom_check(oneswitch->bloom, playload, 20));
            if(k > 28 && playload != NULL && bloom_check(oneswitch->bloom, playload, 20)) {
				VLOG_INFO("--- dedu a chunk ---");

            	//modify the size of packet(struct ofpbuf)
                packet->size=(packet->l7 - packet->l2) + 28;  // remain the unit header only in the gram .
			
                //modify the totle len in the struct of ip
                *((unsigned short int *)(packet->l3)+1)=htons((unsigned short int)(packet->size-14));
					   
                //rewrite the length of udp
                *((unsigned short int *)(packet->l4)+2)=htons((unsigned short int)(packet->l7-packet->l4+28));
    ...
    if (!facet) {
        struct rule_dpif *rule = rule_dpif_lookup(ofproto, &miss->flow);
	     //VLOG_INFO("flow:%s,priority:%d",flow_to_string(&miss->flow), rule->up.cr.priority);	
